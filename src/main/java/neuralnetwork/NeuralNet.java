...
public class NeuralNet {
    ...
    private void calculateErrors(float[] answers) {
            ...
            if (i == errors.length - 1) {
                ...
                for (int j = 0; j < errors[i].length; j++) {
                    ...
                    for (int k = 0; k < weights[i + 1][j].length; k++) {
                        temp += weights[i + 1][j][k] * errors[i + 1][k];
                    }
            ...
            for (int i = gradients.length-1; i >=0; i--) {
                for (int j = 0; j < gradients[i].length; j++) {
                    for (int k = 0; k < gradients[i][j].length; k++) {
                        gradients[i][j][k] = errors[i][k] * outputs[i][j] * learningRate;
                    }
                }
            }
        }
        ...
        public void train ( int epochSize, float learningRate, float[][] trainingData, String trainingDataName,
        float[][] answers, boolean classification){
            for (int i = 0; i < epochSize; i++) {
                ...
                //System.out.println("guess: "+getOutputsFromLastLayer()[0]+" - random: "+random+" - cost: "+ cost);
